...
8. [결과 리포트](#결과-리포트)
   - [실패 원인 분석](#1-실패-원인-분석)
   - [성능 측정 및 시간 복잡도 $O(N^2)$ 분석](#2-성능-측정-및-시간-복잡도-on2-분석)
...
#### (1) 데이터 및 스키마 검증 실패 (Schema / Validation Error)
...
#### 크기별 MAC 연산 시간 측정 결과 (10회 평균 측정 기준 예시)
...
2. **실측 데이터와 연산 횟수의 상관관계**:
   ...
   - 차수가 높아질수록 I/O overhead를 제외한 纯 CPU 연산 시간이 $N^2$ 곡선에 수렴함을 수치적으로 검증할 수 있음.